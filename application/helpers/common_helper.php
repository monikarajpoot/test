...
function check_userlogin_password($loginname, $pwd, $role) {
	...
	if($role!=''){    
		...
		$result = $ci->db->get_where(USERS, array('puser_email' => $loginname, 'puser_password' => md5($pwd), 'puser_status' => '1'));		
	}
...
function get_order_list($condition = null,$limit = false){
    ...
    $tbl6 = STATE;
    $tbl7 = COUNTRY;
    $CI->db->select("$tbl2.puser_id,puser_fullname as user_name,puser_mobile1 as user_phone,puser_email,puser_role_id,puser_status,puser_register_date,order_id,order_reg_id,
			order_type,datatime,order_status,order_serialize_val, order_payment_mode ,order_amount,order_discription,
			puser_pro_address as user_address,$tbl5.pdistrict_name as area_name,$tbl6.pstatest_name as states,order_used_coupon,order_used_coupone_amount,order_items,order_applied_percent_amount,order_address_billing,order_address_shipping,order_mode,payU_paytm_txnid,order_other_fields,order_cancel_date,order_cancel_reason");
    $CI->db->from($tbl1);
    $CI->db->join($tbl2, "$tbl2.puser_id = $tbl1.order_user_id", 'left');   
    
	$CI->db->join($tbl4, "$tbl4.puser_id = $tbl1.order_user_id", 'left');
	$CI->db->join($tbl5, "$tbl5.pdistrict_id = $tbl4.puser_pro_district", 'left');
	$CI->db->join($tbl6, "$tbl6.pstatet_id = $tbl4.puser_pro_state", 'left');
	$CI->db->join($tbl7, "$tbl7.pcounty_id = $tbl4.puser_pro_country", 'left');
...
function get_product_list($condition = null,$limit = false){
	...
	function get_pharma_delivery_charge($area_pincode){
		$CI = & get_instance();	
		if($area_pincode=='' || $area_pincode==null){
			$get_deliverycharge['area_pincode']=$area_pincode;
			$get_deliverycharge['delivery_charge']=DELIVERY_CHARGE;
			$get_deliverycharge['status']=false;
		}else{
			$get_deliverycharge= get_row("SELECT del_area.pdelev_area_pincode as area_pincode,area_charge.pm_charge_amount AS delivery_charge FROM pm_delivery_area_master  AS del_area
				INNER JOIN `pm_other_charges` AS area_charge ON area_charge.pdelivery_id=del_area.pdelivery_id
				WHERE del_area.pdelev_area_pincode='".$area_pincode."' AND del_area.pdelev_status=1");
				$get_deliverycharge['area_pincode'];
				$get_deliverycharge['delivery_charge'];
				$get_deliverycharge['status']=true;
		}
		return $get_deliverycharge;
	}
...
function get_user_detail($userid,$isadmin=null){
	$CI = & get_instance();
		...
		$tbl1.puser_password,
		$tbl1.puser_add_news,
		$tbl1.puser_edit_news,		
		$tbl1.puser_delete_news");
        $CI->db->from($tbl1);
        $CI->db->join($tbl4, "$tbl4.puser_id = $tbl1.puser_id", 'inner');
        $CI->db->join($tbl6, "$tbl6.puser_role_id = $tbl1.puser_role_id", 'inner');        
        if($userid != null){
...
function send_message_bysms($mobile,$msg){
	...
	$url ="http://smsindia.kolar18.com/submitsms.jsp?user=shubhamb&key=b7359f9decXX&mobile=$mobile&message=$message&senderid=HEALTH&accusage=1";
...
function get_recommandation_health_lists($param){
    $CI = & get_instance();
    $recomandation_master=  'pm_'.RECOMMANDATION_HLTH_MASTER;
    $sql_str="SELECT pm_recomm_id,pm_recomm_title FROM $recomandation_master where pm_recomm_status=1";
    $result= get_rows($sql_str);       
    return $result; 
}
...
function get_menu_tree($parent_id){
    $CI = & get_instance();	
    $menu = '';
    $rows = get_rows("SELECT * FROM pm_news_category where status='1' and parent_id='" .$parent_id . "' ");
    if(!empty($rows)){
        foreach($rows as $row)
        {
			//if($row['parent_id']==0){
			...
			//}else{	
				...
				$last_menu_id = end(explode('/',$row['link_of_menu_id']));
				if($row['is_catogoy_or_menu']==1){					
					$ling_href = base_url().'category/'.$last_menu_id;
				}else{
					$ling_href = base_url().'page/'.$last_menu_id;
				}
				//$ling_href = $row['link_of_menu_id'];
			//}
			
            $menu .="<li><a href='".$ling_href."'>".$row['menu_name']."</a>";
            $tt = get_menu_tree($row['menu_id']);
            if(!empty($tt)){
                $menu .= "<ul class='drop-down one-column hover-fade'>".get_menu_tree($row['menu_id'])."</ul>"; //call  recursively
            }
            $menu .= "</li>";
        }
        return $menu;
...
// upload image
function uploadtypeFile($filename, $path) {
	...
	function get_temp_enq_products(){
        $resulttotal=0;
		$CI = & get_instance();
		$selcted_column="*";
        $CI->db->select($selcted_column);
		$CI->db->from('pm_enquiry_temp');     
		$session_usr_id = $CI->session->userdata('user_id');	
        $CI->db->where("enquiry_user_id",$session_usr_id);;                                  
        $query = $CI->db->get();
        //echo $this->db->last_query();
        if ($query->num_rows() != 0) {
            $result['result'] = $query->result_array();
            ...
            $CI->session->set_userdata('total_enquiry_prd',$resulttotal);
            return $result;
        } else {
			$CI->session->set_userdata('total_enquiry_prd',$resulttotal);
            return FALSE;
	...
	function get_news_with_category($cat_id,$limit,$is_slider){
		$resulttotal=0;
		$CI = & get_instance();
		if($is_slider!='' && $cat_id=='' ){
			$sqlstr = "SELECT news.*,cat.menu_id,cat.menu_name,usr.puser_fullname  
			...
			WHERE is_view_slider_header=1 and news.status='1' $limit";
		}else if(is_string($cat_id) && $limit==null && $is_slider==''){
			$sqlstr = "SELECT news.*,cat.menu_id,cat.menu_name,usr.puser_fullname  
			FROM pm_news_master AS news 
			LEFT JOIN pm_news_category AS cat ON news.menu_child_id=cat.menu_id
			inner join pm_users as usr on usr.puser_id=news.is_login_user
			WHERE cat.show_home_in_sequence!=0 and news.status='1' order by cat.show_home_in_sequence asc";
		}else if(is_numeric($cat_id) && $limit!==null && $is_slider==''){
			$sqlstr = "SELECT news.*,cat.menu_id,cat.menu_name,usr.puser_fullname  
			FROM pm_news_master AS news 
			LEFT JOIN pm_news_category AS cat ON news.menu_child_id=cat.menu_id
			inner join pm_users as usr on usr.puser_id=news.is_login_user
			WHERE cat.menu_id=$cat_id and news.status='1' AND (news.is_show_front=1 || news.is_show_front=0 ) order by news.is_highlight desc $limit";
		}else{
			$sqlstr = "SELECT news.*, cat.menu_id,cat.menu_name,usr.puser_fullname   
			FROM pm_news_master AS news 
			LEFT JOIN pm_news_category AS cat ON news.menu_child_id=cat.menu_id
			inner join pm_users as usr on usr.puser_id=news.is_login_user
			WHERE news.status='1' AND menu_child_id=$cat_id $limit";
		}
		$result_array= get_rows($sqlstr);		
		return $result_array;
	}
	function get_news_post($task,$limit){
		$resulttotal=0;
		$CI = & get_instance();
		if($task=='get_latest_post'){
			$sqlstr = "SELECT news.*,cat.menu_id,cat.menu_name,usr.puser_fullname  
			FROM pm_news_master AS news 
			LEFT JOIN pm_news_category AS cat ON news.menu_child_id=cat.menu_id
			inner join pm_users as usr on usr.puser_id=news.is_login_user
			WHERE news.status='1' order by news.id desc $limit";
		}else if($task=='get_feature_post'){
			$sqlstr = "SELECT news.*,cat.menu_id,cat.menu_name,usr.puser_fullname  
			FROM pm_news_master AS news 
			LEFT JOIN pm_news_category AS cat ON news.menu_child_id=cat.menu_id
			inner join pm_users as usr on usr.puser_id=news.is_login_user
			WHERE news.status='1' and news.is_view_feature_widget='1' order by news.updated_date desc $limit";
		}
		$result_array= get_rows($sqlstr);		
		return $result_array;
	}
	function get_user_permision(){
	   $ci = & get_instance();	       
		$user_permision_array=array();
		$user_permision_array['is_add_news']=$ci->session->userdata['is_add_news'];
		$user_permision_array['is_edit_news']=$ci->session->userdata['is_edit_news'];
		$user_permision_array['is_delete_news']=$ci->session->userdata['is_delete_news'];		
		return $user_permision_array;
	}
