            ...
            <div class="box">
                <div class="box-header">
                    ...
                    <div class="box-tools pull-right">
                        <a href="<?php echo base_url('admin')?>/add_news" class="btn btn-primary">Add News</a>                    
                ...
                <div class="box-body">
                    ...
                    <table  class="table table-bordered table-striped">
                        ...
                        <tr ng-if="all_products!='false'"  ng-repeat="all_product in all_products  | filter:searchFish">
                            ...
                            <td>{{all_product.title}}</td>
                            <td>{{all_product.cat_name}}</td>
                            <td>{{all_product.description | htmlToPlaintext | strLimit:100}}</td>
                            <td>{{all_product.pm_create_date_time | dateToISO}}</td>
                            <td>
                                <div class="btn-group">
                                    <a href="<?php echo base_url();?>admin/news/view/{{all_product.pm_master_id}}" class="btn btn-sm btn-twitter"></i> View</a>
                                    <a href="<?php echo base_url();?>admin/update_news/{{all_product.pm_master_id}}" class="btn btn-sm btn-twitter"></i> Update</a>
...
<script>
    ...
    myApp.controller('myCtrl', function($scope, $http) {
        ...
        getData();
        function getData() {
            var HTTP_PATH2='<?php echo base_url().'manage_admin/manage_news/get_news_list'; ?>';
        ...
        $scope.deleteproduct = function(all_product){
			alert(all_product.pm_master_id);
            var conf = confirm('Are you sure to delete the user?');
            if(conf === true){
                var HTTP_PATH='<?php echo base_url().'manage_admin/manage_news/delete_news'; ?>';
                var PATH = all_product.imagepath;
                var data = $.param({
                    'id': all_product.pm_master_id,
                    'img_path': PATH
                });
                var config = {
                    headers : {
