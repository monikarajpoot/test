        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    ...
                    <a href="<?php echo base_url('admin'); ?>/news_list" class="btn btn-primary" title="View List" style="float:right"><i class="fa fa-fw fa-list-ul"></i>View All NEWS</a>
                </div>
            </div><!-- /.box-header -->
        </div>
                    ...
                    <div class="box-body">
                        <div class="col-md-8">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control"  name="pt_child_id" id="pt_child_id"  ng-model="myFile.pt_child_id" ng-change="setval(myFile.pt_child_id)" required>
                                    <option value="">Select Cetegory</option>
                                    <?php foreach ($get_menu as $key => $menu) { ?>
										<?php //if($menu['parent_id']!='0'){ ?>
											<option value="<?php echo $menu['menu_id'] ?>" > <?php echo $menu['menu_name']; ?> (<?php echo ($menu['parent_id']==0? 'Parent': $menu['parent_nm']); ?>)</option>
										<?php //} ?>
									<?php } ?>
                                </select>
...
<script>
    ...
    myApp.controller('myCtrl', ['$scope', 'fileUpload', function($scope, fileUpload){
        $scope.uploadFile = function(){
            ...
            var uploadUrl = '<?php echo base_url().'manage_admin/manage_news/add_news'; ?>';
            var redirect = '<?php echo base_url().'admin/news_list'; ?>';
            fileUpload.uploadFileToUrl(file,uploadUrl,redirect);
        };

        $scope.setval = function(pt_child_id){
            var menu1s = <?php echo json_encode($get_menu); ?>;			
			/console.log(menu1s);*/
            $scope.myFile.pt_parent_id =  null;
            angular.forEach(menu1s, function(menu1, key) {   
				if($scope.myFile.news_cat_id){
					$scope.myFile.pt_parent_id = menu1.parent_id;					
					console.log($scope.myFile.pt_parent_id);
				}else if(menu1.menu_id == pt_child_id){
					$scope.myFile.pt_parent_id = menu1.parent_id;
					console.log($scope.myFile.pt_parent_id);
                }
            });
        ...
        $scope.modify_uploadFile = function(){
            ...
            file.pt_description = CKEDITOR.instances.pt_description.getData();
            var redirect = '<?php echo base_url().'admin/news_list'; ?>';
            var uploadUrl = '<?php echo base_url().'manage_admin/manage_news/update_news'; ?>';
            fileUpload.uploadFileToUrl(file,uploadUrl,redirect);
        };

        var pt_detail = <?php echo json_encode(@$pt_detail['pm_master_id'] ? $pt_detail : ''); ?>;
        if(pt_detail != ''){
            $scope.myFile = {} ;
            $scope.myFile.pt_hidden_id = pt_detail.pm_master_id ;
            $scope.myFile.pt_child_id = pt_detail.news_cat_id ;
